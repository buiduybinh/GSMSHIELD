...
{
	char *SMS_time;
	return GetSMS(position,phone_number,SMS_time,SMS_text,max_SMS_len);
}
char SMSGSM::GetSMS(byte position, char *phone_number, char *SMS_time, char *SMS_text, byte max_SMS_len) 
{
  char ret_val = -1;
  char *p_char; 
  char *p_char1;
  ...
  switch (gsm.WaitResp(5000, 100, "+CMGR")) {
    ...
    case RX_FINISHED_STR_RECV:
      ...
      if (p_char != NULL) {
		strncpy(SMS_time, (char *) (p_char + 6),17);
...
char SMSGSM::DeleteAllSMS(void) 
{
  ...
  //send "AT+CMGDA=XY" - where XY = position
  gsm.SimpleWrite(F("AT+CMGDA=\""));
